...
export default function Banner({description, title, shortTitle}: {description: string, title: string, shortTitle: string}) {
    ...
    return (
        <div style={{height: "390px",  backgroundImage: `url('${assetsInfo.bannerImage}')`, backgroundRepeat: "no-repeat", backgroundSize: "cover", backgroundColor: "grey"}} className={`w-100 position-relative ${styles.bannerPattern}`} ref={parallax.ref as LegacyRef<HTMLDivElement>}>
            ...
            <div className='position-absolute start-50 top-50 translate-middle d-flex flex-column justify-content-center algin-items-center gap-2'>
                ...
                <p className='text-truncate text-wrap text-center text-white text-center overflow-hidden h-25'>
                    {description}
                </p>
            </div>
        </div>
